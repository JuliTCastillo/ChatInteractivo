...
io.on('connection', socket => {
    ...
    socket.on("tateti:start", username => {
        if (tateti.isFull()) {
            socket.emit("tateti:full", "La partida es llena!")
            return;
        }

        const ok = tateti.addPlayer(socket.id, username); //Agregamos el jugador
        if (!ok) {
            socket.emit("tateti:error", "No se pudo unir al jugador");
            return;
        }

        if (tateti.isFull()) {
            tateti.players.forEach(p => {
                io.to(p.id).emit("tateti:players", {
                    players: tateti.players,  
                    board: tateti.board,
                    turn: tateti.turn
                });
            });
